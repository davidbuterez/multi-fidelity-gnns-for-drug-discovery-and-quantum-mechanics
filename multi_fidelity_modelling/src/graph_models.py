...
from torch_geometric.nn import GCNConv, PNAConv, VGAE, global_mean_pool, global_add_pool, global_max_pool, GINConv, GINEConv
...
class VariationalGCNEncoder(pl.LightningModule):
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                modules.append((GCNConv(in_channels, intermediate_dim, cached=False), 'x, edge_index -> x'))
            else:
                modules.append((GCNConv(intermediate_dim, intermediate_dim, cached=False), 'x, edge_index -> x'))
...
class VariationalGINEncoder(pl.LightningModule):
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int, edge_dim: int=None, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                if self.edge_dim:
                    modules.append((GINEConv(nn.Sequential(Linear(in_channels, intermediate_dim), ReLU(), Linear(intermediate_dim, intermediate_dim)), edge_dim=self.edge_dim), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((GINConv(nn.Sequential(Linear(in_channels, intermediate_dim), ReLU(), Linear(intermediate_dim, intermediate_dim))), 'x, edge_index -> x'))
            else:
                if self.edge_dim:
                    modules.append((GINEConv(nn.Sequential(Linear(intermediate_dim, intermediate_dim), ReLU(), Linear(intermediate_dim, intermediate_dim)), edge_dim=self.edge_dim), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((GINConv(nn.Sequential(Linear(intermediate_dim, intermediate_dim), ReLU(), Linear(intermediate_dim, intermediate_dim))), 'x, edge_index -> x'))
        ...
        nn_mu = nn.Sequential(Linear(intermediate_dim, out_channels), ReLU(), Linear(out_channels, out_channels))
        if self.edge_dim:
            self.conv_mu = GINEConv(nn_mu, edge_dim=self.edge_dim)
        else:
            self.conv_mu = GINConv(nn_mu)

        nn_sigma = nn.Sequential(Linear(intermediate_dim, out_channels), ReLU(), Linear(out_channels, out_channels))
        if self.edge_dim:
...
class VariationalPNAEncoder(pl.LightningModule):
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int, train_dataset, edge_dim: int=None, name: str=None):
        ...
        # deg = torch.from_numpy(np.load(f'/projects/cc/kfjs289/AZ_HTS/PNA_degrees/{name}.npy')).to(self.device)

        pna_num_towers = 5

        pna_common_args = dict(aggregators=aggregators, scalers=scalers, deg=deg, edge_dim=None, towers=pna_num_towers,
        ...
        for i in range(self.num_layers):
            if i == 0:
                if self.edge_dim:
                    modules.append((PNAConv(in_channels=in_channels, out_channels=intermediate_dim, **pna_common_args), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((PNAConv(in_channels=in_channels, out_channels=intermediate_dim, **pna_common_args), 'x, edge_index -> x'))
            else:
                if self.edge_dim:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=intermediate_dim, **pna_common_args), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=intermediate_dim, **pna_common_args), 'x, edge_index -> x'))
        ...
        self.conv_mu = PNAConv(in_channels=intermediate_dim, out_channels=out_channels, **pna_common_args)
        self.conv_logstd = PNAConv(in_channels=intermediate_dim, out_channels=out_channels, **pna_common_args)
    ...
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                modules.append((GCNConv(intermediate_dim, out_channels, cached=False), 'x, edge_index -> x'))
    ...
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int, edge_dim: int=None, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                if self.edge_dim:
                    modules.append((GINEConv(nn.Sequential(Linear(intermediate_dim, intermediate_dim), ReLU(), Linear(intermediate_dim, out_channels)), edge_dim=self.edge_dim), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((GINConv(nn.Sequential(Linear(intermediate_dim, intermediate_dim), ReLU(), Linear(intermediate_dim, out_channels))), 'x, edge_index -> x'))
    ...
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int, train_dataset, edge_dim: int=None, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                if self.edge_dim:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=out_channels, **pna_common_args), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=out_channels, **pna_common_args), 'x, edge_index -> x'))
...
class Estimator(pl.LightningModule):
    def __init__(self, task_type: str, num_features: int, gnn_intermediate_dim: int, node_latent_dim: int, graph_latent_dim: int, \
                 train_dataset=None, batch_size: int=32, lr: float=0.001, conv_type: str='GCN', linear_output_size: int=1, \
                 auxiliary_dim: int=0, output_intermediate_dim: int=768, readout: str='linear', max_num_atoms_in_mol: int=55, \
                 scaler=None, monitor_loss: str='val_total_loss', record_val_embeddings_every_n_epochs: int=None, \
                 num_layers: int=None, use_batch_norm: bool=False, set_transformer_hidden_dim: int=None, \
                 set_transformer_num_heads: int=None, edge_dim: int=None, use_vgae: bool=True, name: str=None):
        super().__init__()
        assert task_type in ['classification', 'regression']
        assert conv_type in ['GCN', 'GIN', 'PNA']
        assert readout in ['linear', 'global_mean_pool', 'global_add_pool', 'global_max_pool', 'set_transformer']

        print('%s task with %d %s layers and %s readout.' % (task_type.capitalize(), num_layers, conv_type, readout))
        ...
        self.edge_dim = edge_dim
        self.record_val_embeddings_every_n_epochs = record_val_embeddings_every_n_epochs
        self.graph_latent_dim = graph_latent_dim if (readout == 'linear' or readout == 'set_transformer') else node_latent_dim
        self.task_type = task_type
        self.global_pool_fn = global_mean_pool if readout == 'global_mean_pool' else (global_add_pool if readout == 'global_add_pool' else (global_max_pool if readout == 'global_max_pool' else None))
        ...
        if self.global_pool_fn:
            ...
            print('Using %d latent node features and %d latent graph features.' % (node_latent_dim, graph_latent_dim))

        self.auxiliary_dim = auxiliary_dim if auxiliary_dim else 0
        if self.auxiliary_dim > 0:
            print('Using auxiliary data with dimension %d, total with GNN/VGAE embeddings: %d.' % (self.auxiliary_dim, graph_latent_dim + self.auxiliary_dim))
        ...
        self.metric_fn = get_cls_metrics if task_type == 'classification' else get_metrics
        ...
        gnn_args = dict(in_channels=num_features, out_channels=node_latent_dim, intermediate_dim=self.gnn_intermediate_dim,
                        use_batch_norm=self.use_batch_norm, num_layers=self.num_layers, name=name)
        if self.edge_dim:
        ...
        # When using global pooling we do not even define the linear readout layers
        if self.global_pool_fn:
            self.linear_output1 = nn.Linear(node_latent_dim + self.auxiliary_dim, 64)
        elif self.readout == 'linear':
            self.linear_readout1 = nn.Linear(self.max_num_atoms_in_mol * node_latent_dim, self.gnn_intermediate_dim)
            self.linear_readout2 = nn.Linear(self.gnn_intermediate_dim, graph_latent_dim)
            if self.use_batch_norm:
                self.bn1 = nn.BatchNorm1d(self.gnn_intermediate_dim)
                self.bn2 = nn.BatchNorm1d(graph_latent_dim)


        self.linear_output1 = nn.Linear(self.graph_latent_dim + self.auxiliary_dim, 32)

        if self.use_batch_norm:
            self.bn3 = nn.BatchNorm1d(32)

        self.linear_output2 = nn.Linear(32, self.linear_output_size)

        ### Enabling this caused errors in the past.
        # Save hyperparameters function provided by PyTorch Lightning
        # self.save_hyperparameters()


    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch: torch.Tensor, aux_data: Optional[torch.Tensor]=None, edge_attr: Optional[torch.Tensor]=None):
        ...
        if not self.global_pool_fn and self.readout == 'linear':
            graph_embeddings, _ = to_dense_batch(z, batch, fill_value=0, max_num_nodes=self.max_num_atoms_in_mol)

            # Reshape to (current_batch_shape, flattened_node_features)
            graph_embeddings = graph_embeddings.reshape(graph_embeddings.shape[0], self.max_num_atoms_in_mol * self.node_latent_dim)

            # Apply the dense layers to get a graph-level representation
            if self.use_batch_norm:
                graph_embeddings = self.bn1(self.linear_readout1(graph_embeddings)).relu()
                graph_embeddings_without_relu = self.bn2(self.linear_readout2(graph_embeddings))
            else:
        ...
        elif not self.global_pool_fn and self.readout == 'set_transformer':
            graph_embeddings, _ = to_dense_batch(z, batch, fill_value=0, max_num_nodes=self.max_num_atoms_in_mol)
            graph_embeddings = self.st(graph_embeddings)
            graph_embeddings_to_return = graph_embeddings
            graph_embeddings = graph_embeddings.mean(dim=1)
            graph_embeddings = graph_embeddings.relu()
        ...
        if self.auxiliary_dim > 0:
            ...
            if self.auxiliary_dim == 1:
                ...
                aux_data = aux_data.reshape((graph_embeddings.shape[0], self.auxiliary_dim))

            # Actual concatenation
            graph_embeddings = torch.cat((graph_embeddings, aux_data), dim=1)

        # 3. Apply a final classifier/regression network
        # If not using 'linear' readout then linear_readout%d and bn%d for %d in [1, 2] are not used
    ...
    def configure_optimizers(self):
        ...
        return {
            'optimizer': opt,
            'lr_scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(opt, factor=0.5, patience=20),
            'monitor': self.monitor_loss
        }


    def _batch_loss(self, x: torch.Tensor, edge_index: torch.Tensor, y: Optional[torch.Tensor]=None, batch_mapping: Optional[torch.Tensor]=None, aux_data: Optional[torch.Tensor]=None, edge_attr: Optional[torch.Tensor]=None):
        # Number of nodes in graph
        num_nodes = x.shape[0]

        # Forward pass (graph_embeddings not used here so far, after forward)
        if not self.edge_dim:
            z, graph_embeddings, predictions = self.forward(x, edge_index, batch_mapping, aux_data)
        else:
            z, graph_embeddings, predictions = self.forward(x, edge_index, batch_mapping, aux_data, edge_attr)
        ...
        if self.task_type == 'classification':
            ...
            # task_loss = F.cross_entropy(predictions, y.squeeze().long())
            task_loss = F.binary_cross_entropy_with_logits(predictions.squeeze(), y.squeeze().float())
    ...
    def _step(self, batch: torch.Tensor, step_type: str):
        ...
        x, edge_index, edge_attr, y, batch_mapping, aux_data = batch.x, batch.edge_index, batch.edge_attr, batch.y, batch.batch, batch.aux_data

        total_loss, vgae_loss, task_loss, z, graph_embeddings, predictions = self._batch_loss(x, edge_index, y, batch_mapping, aux_data, edge_attr)
        ...
        if step_type == 'train':
            ...
            # if self.record_val_embeddings_every_n_epochs and (self.current_epoch + 1) % self.record_val_embeddings_every_n_epochs == 0 and self.current_epoch > 0:
            #     self.val_graph_embeddings[self.current_epoch].append(graph_embeddings)
        elif step_type == 'test':
    ...
    def on_train_start(self):
        # Make PyTorch Lightning happy
        self.log('train_total_loss', 0.0)
    ...
    def _epoch_end_report(self, epoch_outputs):
        def flatten_list_of_tensors(lst):
            return np.array([item.item() for sublist in lst for item in sublist])

        y_pred, y_true = flatten_list_of_tensors([item[0] for item in epoch_outputs]), flatten_list_of_tensors([item[1] for item in epoch_outputs])
        if self.scaler:
        ...
        if self.task_type == 'classification':
            ...
            metrics = self.metric_fn(y_true, y_pred, num_cls=self.linear_output_size)
        else:
    ...
    def on_test_epoch_end(self):
        ...
        self.test_metrics[self.num_called_test], y_pred, y_true = self._epoch_end_report(test_outputs_per_epoch)
        self.test_output[self.num_called_test] = y_pred
        self.test_true[self.num_called_test] = y_true
        self.test_graph_embeddings[self.num_called_test] = torch.cat(self.test_graph_embeddings[self.num_called_test]).detach().cpu().numpy()
        self.num_called_test += 1
