...
def main():
    ...
    gnn_args = dict(task_type=task_type,
    ...
    if argsdict['conv'] == 'PNA':
        pna_args = dict(train_dataset=data_module.dataset, name=argsdict['logging_name'])
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='%s-%s-%s-%s-%s-num_layers=%s-batch_norm=%s-{epoch:03d}-{val_total_loss:.5f}' % (task_type, gnn_type,
                argsdict['conv'], edges, argsdict['target_label'], argsdict['num_layers'], argsdict['use_batch_norm']),
        mode='min',
        save_top_k=1,
    )

    early_stopping_callback = EarlyStopping(monitor=MONITOR_LOSS, mode='min', patience=100)
