...
from graph_models import Estimator
from data_loading import GeometricDataModule
...
def main():
    ...
    parser.add_argument('--load-ckpt-and-generate-embs', action=argparse.BooleanOptionalAction, required=False,
                        default=False)
    ...
    if argsdict['load_ckpt_and_generate_embs'] is not None and argsdict['load_ckpt_and_generate_embs']:
    ...
    data_module = GeometricDataModule(
        ...
        separate_test_path=argsdict['data_path'] if should_load_and_generate_embs else None
    )
    ...
    if should_load_and_generate_embs:
        ...
        np.save(os.path.join(argsdict['out_dir'], 'low_fidelity_predictions.npy'), model.test_output)
        np.save(os.path.join(argsdict['out_dir'], 'low_fidelity_true.npy'), model.test_true)
        np.save(os.path.join(argsdict['out_dir'], 'low_fidelity_graph_embeddings.npy'), model.test_graph_embeddings)
