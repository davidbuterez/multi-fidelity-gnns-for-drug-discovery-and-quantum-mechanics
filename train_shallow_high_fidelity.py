...
def add_label(x_train, x_val, x_test, train_df, val_df, test_df, train_label, eval_label):
    x_train_lbl = np.concatenate((x_train, np.expand_dims(train_df[train_label].values, axis=1)), axis=1)
    x_val_lbl = np.concatenate((x_val, np.expand_dims(val_df[eval_label].values, axis=1)), axis=1)
    x_test_lbl = np.concatenate((x_test, np.expand_dims(test_df[eval_label].values, axis=1)), axis=1)

    return x_train_lbl, x_val_lbl, x_test_lbl


def add_embeddings(x_train, x_val, x_test, train_df, val_df, test_df, embs_label):
    train_embs = np.array([np.array(
            arr.rstrip().lstrip().replace('[', '').replace(']', '').split(','), dtype=float
        ) for arr in train_df[embs_label].values]
    )

    val_embs = np.array([np.array(
        arr.rstrip().lstrip().replace('[', '').replace(']', '').split(','), dtype=float
        ) for arr in val_df[embs_label].values])

    test_embs = np.array([np.array(
        arr.rstrip().lstrip().replace('[', '').replace(']', '').split(','), dtype=float
...
def main():
    ...
    scoring = 'roc_auc' if argsdict['task_type'] == 'classification' else 'neg_mean_squared_error'
    if argsdict['rf_or_svm'] == 'rf':
        print('Using RF.')
        model = RandomForestClassifier if argsdict['task_type'] == 'classification' else RandomForestRegressor
    ...
    if argsdict['task_type'] == 'regression':
        ...
        y_train_val_scaled = np.concatenate([y_train_scaled, y_valid_scaled]).reshape(-1,)
        ...
        y_train_val = np.concatenate([y_train, y_val]).reshape(-1,)
    ...
    if argsdict['lbl_or_emb'] == 'lbl':
        ...
        x_train, x_val, x_test = add_label(
            x_train, x_val, x_test, train_df, val_df, test_df, argsdict['train_SD_label'], argsdict['eval_SD_label']
        )
    ...
    rf_random_grid = {'n_estimators': [50, 100, 150, 200, 250, 300, 500],
    ...
    grid_search_result = perform_grid_search(model, grid, ps, scoring, x_train_val, y_train_val)
    ...
    preds, estimator = train_model(model, x_train, y_train, x_test, parameters=None, model_type=argsdict['rf_or_svm'])
