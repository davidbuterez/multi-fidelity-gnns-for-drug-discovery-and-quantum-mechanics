...
from reporting import get_metrics, get_metrics_pt, get_cls_metrics
...
class VariationalGCNEncoder(pl.LightningModule):
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int,
                name: str=None):
...
class VariationalGINEncoder(pl.LightningModule):
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int,
                edge_dim: int=None, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                if self.edge_dim:
                    modules.append((
                        GINEConv(nn.Sequential(Linear(in_channels, intermediate_dim), ReLU(), 
                                            Linear(intermediate_dim, intermediate_dim)
                                            ), 
                                edge_dim=self.edge_dim), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((
                        GINConv(nn.Sequential(Linear(in_channels, intermediate_dim), ReLU(),
                                            Linear(intermediate_dim, intermediate_dim))
                                            ), 'x, edge_index -> x'))
            else:
                if self.edge_dim:
                    modules.append((
                        GINEConv(nn.Sequential(Linear(intermediate_dim, intermediate_dim), ReLU(), 
                                            Linear(intermediate_dim, intermediate_dim)
                                            ), edge_dim=self.edge_dim), 'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((
                        GINConv(nn.Sequential(Linear(intermediate_dim, intermediate_dim), ReLU(),
                                            Linear(intermediate_dim, intermediate_dim))
                                            ), 'x, edge_index -> x'))
...
class VariationalPNAEncoder(pl.LightningModule):
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int,
                    train_dataset, edge_dim: int=None, name: str=None):
        ...
        for i in range(self.num_layers):
            if i == 0:
                if self.edge_dim:
                    modules.append((PNAConv(in_channels=in_channels, out_channels=intermediate_dim, **pna_common_args),
                ...
                if self.edge_dim:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=intermediate_dim, **pna_common_args),
                                    'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=intermediate_dim, **pna_common_args),
                                    'x, edge_index -> x'))
    ...
    def __init__(self, in_channels: int, intermediate_dim: int, use_batch_norm: bool, out_channels:int, num_layers: int,
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                if self.edge_dim:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=out_channels, **pna_common_args),
                                    'x, edge_index, edge_attr -> x'))
                else:
                    modules.append((PNAConv(in_channels=intermediate_dim, out_channels=out_channels, **pna_common_args),
                                    'x, edge_index -> x'))
...
class Estimator(pl.LightningModule):
    def __init__(self, 
            task_type: str, num_features: int, gnn_intermediate_dim: int, node_latent_dim: int,
            graph_latent_dim: Optional[int]=None, train_dataset=None, batch_size: int=32, lr: float=0.001,
            linear_output_size: int=1, auxiliary_dim: int=0, output_intermediate_dim: int=768, scaler=None,
            readout: str='linear', max_num_atoms_in_mol: int=55, monitor_loss: str='val_total_loss',
            num_layers: Optional[int]=None, use_batch_norm: bool=False, name: Optional[str]=None,
            set_transformer_hidden_dim: Optional[int]=None, set_transformer_num_heads: Optional[int]=None,
            set_transformer_num_sabs: Optional[int]=None, set_transformer_dropout: float=0.0,
            edge_dim: Optional[int]=None, use_vgae: bool=True, linear_interim_dim: int=64, linear_dropout_p: float=0.2,
            conv_type: str='GCN', only_train: bool=False,
        ):
        ...
        self.graph_latent_dim = (
            graph_latent_dim if self.only_train else node_latent_dim
        )
        self.task_type = task_type
        self.global_pool_fn = (global_mean_pool if readout == 'global_mean_pool' else 
                              (global_add_pool if readout == 'global_add_pool' else 
                              (global_max_pool if readout == 'global_max_pool' else None)))
        ...
        if self.global_pool_fn:
            ...
            print('Using %d latent node features and %d latent graph features.' % (node_latent_dim, self.graph_latent_dim))

        self.auxiliary_dim = auxiliary_dim if auxiliary_dim else 0
        if self.auxiliary_dim > 0:
            print('Using auxiliary data with dimension %d, total with GNN/VGAE embeddings: %d.' % (
        ...
        gnn_args = dict(in_channels=num_features, out_channels=node_latent_dim, num_layers=self.num_layers,
                        intermediate_dim=self.gnn_intermediate_dim, use_batch_norm=self.use_batch_norm,
                        name=self.name)
        ...
        if self.readout == 'linear':
            self.linear_readout1 = nn.Linear(self.max_num_atoms_in_mol * node_latent_dim, self.linear_interim_dim)
            self.linear_readout2 = nn.Linear(self.linear_interim_dim, self.graph_latent_dim)
            if self.use_batch_norm:
            ...
            self.st = SetTransformer(
                dim_input=node_latent_dim, num_outputs=32,
                dim_output=self.graph_latent_dim, num_inds=None, ln=True,
                dim_hidden=self.set_transformer_hidden_dim, num_heads=self.set_transformer_num_heads,
                num_sabs=self.set_transformer_num_sabs, dropout=self.set_transformer_dropout
            )

        if self.only_train:
            self.linear_output1 = nn.Linear(self.graph_latent_dim + self.auxiliary_dim, 256)
        else:
            self.linear_output1 = nn.Linear(self.node_latent_dim * 3 + self.auxiliary_dim, 256)
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch: torch.Tensor,
                aux_data: Optional[torch.Tensor]=None, edge_attr: Optional[torch.Tensor]=None):
        ...
        elif self.only_train and not self.global_pool_fn and self.readout == 'set_transformer':
            graph_embeddings, _ = to_dense_batch(z, batch, fill_value=0, max_num_nodes=self.max_num_atoms_in_mol)
        ...
        if not self.only_train:
            ...
            graph_embeddings = torch.cat((graph_embeddings_sum, graph_embeddings_mean, graph_embeddings_max), dim=-1)
            graph_embeddings_to_return = graph_embeddings
    ...
    def configure_optimizers(self):
        ...
        return {
            'optimizer': opt,
            'lr_scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(opt, factor=0.75, patience=15),
            'monitor': self.monitor_loss
        }


    def _batch_loss(self, x: torch.Tensor, edge_index: torch.Tensor, y: Optional[torch.Tensor]=None,
                    batch_mapping: Optional[torch.Tensor]=None, aux_data: Optional[torch.Tensor]=None,
    ...
    def _step(self, batch: torch.Tensor, step_type: str):
        ...
        x, edge_index, edge_attr, y, batch_mapping = batch.x, batch.edge_index, batch.edge_attr, batch.y, batch.batch
        aux_data = batch.aux_data

        total_loss, vgae_loss, task_loss, _, graph_embeddings, predictions = self._batch_loss(
            x, edge_index, y, batch_mapping, aux_data, edge_attr
        )
    ...
    def _epoch_end_report(self, epoch_outputs, epoch_type):
        ...
        if self.task_type == 'classification':
            ...
            if self.only_train:
                ...
                self.log(f'{epoch_type} R2 {self.name}', metrics[-1], batch_size=self.batch_size)
                self.log(f'{epoch_type} MAE {self.name}', metrics[0], batch_size=self.batch_size)
            else:
                ...
                for metric_name, metric_value in metrics.items():
                    self.log(f'{epoch_type} {metric_name}', metric_value, batch_size=self.batch_size)
    ...
    def on_validation_epoch_end(self):
        if not self.only_train:
            ...
            self.val_metrics[self.current_epoch], y_pred, y_true = self._epoch_end_report(
                val_outputs_per_epoch, epoch_type='Validation'
            )
