...
class GraphMoleculeDataset(TorchDataset):
    def __init__(self, csv_path: str, max_atom_num: int, smiles_column_name: str,
                 label_column_name: Union[str, List[str]], auxiliary_data_column_name: Optional[str]=None,
                 lbl_or_emb: str='lbl', scaler: Optional[StandardScaler]=None, id_column: Optional[str]=None):
    ...
    def __getitem__(self, idx: Union[torch.Tensor, slice, List]):
        ...
        if self.auxiliary_data_column_name:
            ...
            if self.lbl_or_emb and self.lbl_or_emb == 'lbl':
                ...
                aux_data = torch.Tensor(
                    np.stack(np.fromstring(selected[self.auxiliary_data_column_name].values[0][1:-1], sep=', '))
                )
        else:
            aux_data = None
        ...
        geometric_data_points = [GeometricData(x=atom_feat[i], edge_attr=bond_feat[i], edge_index=edge_index[i],
                                               y=labels[i], aux_data=aux_data,
                                               iden=ids[i]) for i in range(len(atom_feat))]
...
class GeometricDataModule(pl.LightningDataModule):
    def __init__(self, batch_size: int, seed: int, max_atom_num: int=80, split: Tuple[float, float]=(0.9, 0.05),
                 train_path: Optional[str]=None, separate_valid_path: Optional[str]=None,
                 separate_test_path: Optional[str]=None, id_column: Optional[str]=None,
                 num_cores: Tuple[int, int, int]=(12, 0, 12), smiles_column_name: str='SMILES',
                 label_column_name: str='SD', train_auxiliary_data_column_name: Optional[str]=None,
                 lbl_or_emb: str='lbl', eval_auxiliary_data_column_name: Optional[str]=None, use_standard_scaler=False):
    ...
    def train_dataloader(self, shuffle=True):
        if self.train:
            return GeometricDataLoader(self.train, self.batch_size, shuffle=shuffle, num_workers=self.num_cores[0],
                                       pin_memory=True,
                                    )
        return None
    ...
    def val_dataloader(self):
        return GeometricDataLoader(self.val, self.batch_size, shuffle=False, pin_memory=True,
    ...
    def test_dataloader(self):
        if self.test:
            return GeometricDataLoader(self.test, self.batch_size, shuffle=False, pin_memory=True,
